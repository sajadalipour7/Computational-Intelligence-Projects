...
class Player(pygame.sprite.Sprite):
    def __init__(self, game_mode):
        ...
        if self.game_mode == "Neuroevolution":
            ...
            layer_sizes = [3,10,1]  # TODO (Design your architecture here by changing the values)
    ...
    def think(self, screen_width, screen_height, obstacles, player_x, player_y):
        ...
        input_vector=self.create_input_vector(player_x,player_y,obstacles)
        
        ans=self.nn.forward(input_vector)
        # print(ans)
        if ans[0][0]>0.5:
    ...
    def create_input_vector(self,player_x,player_y,obstacles):
        method="first"
        if method=="first":
            vector=np.zeros((3,1))
            vector[0][0]=(player_x-177)/211.0
            if len(obstacles)==0:
                # print(vector)
                return vector
            x=0
            ...
            vector[1][0]=(x-177)/233.0
            vector[2][0]=(640-y+100)/756.0
            # print(vector)
            return vector
        elif method=="second":
            vector=np.zeros((5,1))
            ...
            for i in range(0,len(obstacles)):
                ...
                if yTmp<640 :
                    if not firstPass:
                        ...
                        break
            if x1!=0 and y1!=0:
                vector[1][0]=(x1-177)/233.0
                vector[2][0]=(640-y1+100)/756.0
            if x2!=0 and y2!=0:
                vector[3][0]=(x2-177)/233.0
                vector[4][0]=(640-y2+100)/756.0
            # print(vector)
            return vector
    ...
    def update(self):
        ...
        if self.game_mode == "Manual":
            self.player_input()
            for obsta in global_variables['obstacle_groups']:
                break
