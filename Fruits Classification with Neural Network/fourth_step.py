...
class NeuralNetworkFourthStep:
    ...
    def accuracy(self):
        ...
        print("Accuracy for fourth step is : " + str(1.0 * correct_predictions / len(self.x_set)))
        return 1.0*correct_predictions/len(self.x_set)


train_set_for_fourth_step = train_set[:200]
x_set_for_fourth_step = []
y_set_for_fourth_step = []
for i in range(0, len(train_set_for_fourth_step)):
    x_set_for_fourth_step.append(train_set_for_fourth_step[i][0])
    y_set_for_fourth_step.append(train_set_for_fourth_step[i][1])

average_accuracy=0
for i in range(0,10):
    neural_network_fourthStep = NeuralNetworkFourthStep(x_set_for_fourth_step, y_set_for_fourth_step)
    average_accuracy+=neural_network_fourthStep.train_data()
average_accuracy/=10
print(f"Average accuracy is : {average_accuracy*100} %")
